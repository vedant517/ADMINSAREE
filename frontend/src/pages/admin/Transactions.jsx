...
function StatCard({ title, value, icon: Icon, color, sub, trend, trendUp }) {
  return (
    <div className="bg-white rounded-2xl border border-slate-100 p-5 shadow-sm">
        ...
        {trend && (
          <span
            className={`text-xs font-extrabold px-2 py-0.5 rounded-full flex items-center gap-0.5 ${
              trendUp ? 'bg-green-50 text-emerald-600' : 'bg-red-50 text-rose-600'
            }`}
          >
      ...
      <div className="text-xs font-bold text-slate-400 mt-1 uppercase tracking-widest">{title}</div>
        ...
        <div className="flex flex-col gap-3">
          ...
          {transaction.order && (
            <div className="flex justify-between items-center py-2 border-b border-slate-50">
              ...
              <span className="text-xs font-extrabold text-emerald-600">{transaction.order?.orderId || transaction.order}</span>
            </div>
          )}
        </div>
          ...
          <div className="overflow-x-auto min-h-[300px]">
            {isLoading ? (
              <div className="py-16 flex flex-col items-center gap-4">
                <div
                  className="w-11 h-11 border-4 border-indigo-100 border-t-indigo-500 rounded-full"
                  ...
                  <thead>
                    <tr className="bg-gray-50 border-b border-slate-100">
                      {['#', 'Transaction ID', 'Razorpay Order', 'Amount', 'Method', 'Date', 'Status', 'Action'].map((h, i) => (
                        <th
                          key={h}
                          className={`px-3.5 py-3.5 text-xs font-extrabold text-indigo-500 uppercase tracking-wide whitespace-nowrap ${
                      ...
                      filtered.map((t, i) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="px-3.5 py-3.5 text-xs font-extrabold text-indigo-500 font-mono whitespace-nowrap">
                              {t.transactionId}
                            </td>
                            ...
                            <td className="px-3.5 py-3.5 text-center">
                              {(() => {
                                ...
                                return (
                                  <span className={`text-xs font-bold px-2.5 py-1 rounded-full uppercase whitespace-nowrap ${
                                    isCod ? 'text-amber-700 bg-amber-50' : 'text-blue-800 bg-blue-100'
                                  }`}>
                                    {isCod ? 'COD' : 'Razorpay'}
                                  </span>
                                );
                            ...
                            <td className="px-3.5 py-3.5 text-center">
                              <button
                                ...
                                className="p-1.5 bg-transparent border-0 cursor-pointer text-slate-300 rounded-md transition-all hover:text-indigo-500 hover:bg-indigo-50"
                              >
                                <Eye size={16} />
                              </button>
                            </td>
